...
def load_workflow_config(config_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
    if config_path:
        path = Path(config_path)
    else:
        path = CONFIG_DIR / CONFIG_WORKFLOW
    ...
    defaults = {
        "workflow": {
            "enable": True,
            "poll_interval": 1.0,
            "task_default_timeout": 300.0,
            "auto_evaluate": True,
            "log_every_n_iterations": 10,
            "poll_sleep_multiplier": 2.0,
        },
        "execution": {
            "max_execution_time": 1000.0,
            "max_iterations": None,
        }
    }
    ...
    for key in ["workflow", "execution"]:
        if key in config:
            result[key] = {**defaults.get(key, {}), **config[key]}
