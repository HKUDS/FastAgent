# FastAgent Local Server (Desktop Version)

## 1. Introduction

The FastAgent Local Server is a **lightweight, cross-platform** Flask service that launches on the host workstation and exposes a uniform HTTP interface for controlling the native desktop environment. By translating REST calls into deterministic GUI actions—mouse and keyboard synthesis, window management, screenshot capture, file I/O—it enables higher-level FastAgent agents to interact with real software instead of simulated environments.
