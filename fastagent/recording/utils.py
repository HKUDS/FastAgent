...
def load_recording_session(recording_dir: str) -> Dict[str, Any]:
    ...
    # Load kanban snapshots
    snapshots_dir = recording_path / "kanban_snapshots"
    if snapshots_dir.exists():
        for snapshot_file in sorted(snapshots_dir.glob("snapshot_*.json")):
            try:
                with open(snapshot_file, 'r', encoding='utf-8') as f:
                    session["kanban_snapshots"].append(json.load(f))
            except Exception as e:
...
def generate_summary_report(recording_dir: str, output_file: Optional[str] = None) -> str:
    ...
    # Kanban snapshots section
    if session["kanban_snapshots"]:
        lines.append(f"## Kanban Snapshots ({len(session['kanban_snapshots'])} total)")
        lines.append(f"- First snapshot: {session['kanban_snapshots'][0].get('timestamp', 'N/A')}")
        lines.append(f"- Last snapshot: {session['kanban_snapshots'][-1].get('timestamp', 'N/A')}")
        lines.append("")
