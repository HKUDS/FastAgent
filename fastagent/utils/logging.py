...
def _load_log_level_from_config() -> int:
    """
    Load log_level from config_grounding.json and convert to FASTAGENT_DEBUG value.
    ...
    try:
        ...
        if config_path.exists():
            with open(config_path, 'r', encoding='utf-8') as f:
                ...
                # Convert log level string to FASTAGENT_DEBUG value
                level_map = {
                    "DEBUG": 2,
                    "INFO": 1,
                    "WARNING": 0,
...
FASTAGENT_DEBUG = _load_log_level_from_config()

# Default log directory and file pattern
# Use absolute path to fastagent/logs directory
DEFAULT_LOG_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "logs")
DEFAULT_LOG_FILE_PATTERN = "fastagent_{timestamp}.log"
...
class Logger:
    ...
    3. Dynamically adapts log levels according to ``FASTAGENT_DEBUG``.
    """

    _ROOT_NAME = "fastagent"        # Package root name
    ...
    def _get_default_log_file() -> str:
        ...
        - logs/<script_name>/fastagent_2025-10-24_15-30-00.log
        """
        # Get the name of the main script
        script_name = "fastagent"  # Default name
        try:
            import __main__
    ...
    def get_logger(cls, name: Optional[str] = None) -> logging.Logger:
        """Return a logger with *name* (defaults to ``fastagent``).
    ...
    def configure(
        ...
        *,
        ...
        with cls._lock:
            ...
            # Select the logger to attach handlers to (root logger or fastagent).
            target_logger = (
    ...
    def set_debug(cls, debug_level: int = 2) -> None:
        ...
        global FASTAGENT_DEBUG
        FASTAGENT_DEBUG = max(0, min(debug_level, 2))
    ...
    def add_file_handler(
        ...
        """
        Append a file handler to the given (default ``fastagent``) logger.
    ...
    def _resolve_level(cls, level: Optional[int]) -> int:
        ...
        return {2: logging.DEBUG, 1: logging.INFO}.get(FASTAGENT_DEBUG, logging.WARNING)
...
# ``FASTAGENT_DEBUG`` (preferred) or legacy ``DEBUG`` environment variable.
_env_debug = os.getenv("FASTAGENT_DEBUG") or os.getenv("DEBUG")
...
# Get fastagent logger for internal logging
logger = Logger.get_logger()
logger.debug("FastAgent logging initialized")